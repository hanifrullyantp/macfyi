/** Resolve active promo phase, effective price, countdown end, and slot display for Edge Functions. */
...
export type ResolvePromoInput = {
  ...
  /** Raw JSON from app_settings.promo_plan */
  plan: unknown;
  /** app_settings.promo_slots_remaining */
...
export type ResolvedPromo = {
  ...
  compare_at_idr: number | null;
  ends_at: string | null;
  /** slots_initial dari fase aktif (untuk reset counter di admin) */
  slots_initial_active: number | null;
  /**
   * When non-null, live counter is enabled (webhook may decrement this column).
   * When null, only static slots_initial from phase may be shown.
   */
  slots_remaining: number | null;
  /** What to show visitors: remaining counter, or phase.slots_initial, or null */
...
/** Whether Snap/checkout should reject (when block flag set and counter exhausted). */
export function shouldBlockCheckoutForSlots(r: ResolvedPromo): boolean {
  if (!r.block_checkout_when_slots_zero) return false;
  if (r.slots_remaining === null) return false;
  return r.slots_remaining <= 0;
}
