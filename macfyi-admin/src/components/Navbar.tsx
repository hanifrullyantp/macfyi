...
import { Search, RefreshCw, Bell, User, LogOut } from 'lucide-react';
import { useAdminSession } from "../../../admin-web/src/hooks/useAdminSession";

export const Navbar: React.FC<{ onSignOut?: () => void }> = ({ onSignOut }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0E0E11]/80 backdrop-blur-md border-b border-white/5 h-16 flex items-center justify-between px-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="p-2 text-white/60 hover:text-white hover:bg-white/5 rounded-full transition-all relative">
          <Bell size={20} />
        ...
        {onSignOut ? (
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-full border border-white/10"
          >
            <LogOut size={16} />
            Keluar
          </button>
        ) : null}
        <button className="flex items-center gap-3 p-1.5 pl-3 hover:bg-white/5 rounded-full transition-all border border-transparent hover:border-white/10">
          <div className="text-right hidden sm:block">
            <p className="text-xs font-bold truncate max-w-[140px]">{email || "Admin"}</p>
