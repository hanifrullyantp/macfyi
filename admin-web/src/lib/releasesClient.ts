...
async function invokeEdge<T>(path: string, method: "GET" | "POST", body?: unknown): Promise<T> {
  ...
  if (!res.ok) {
    if (res.status === 403) {
      throw new Error(
        "Akses ditolak. Pastikan user login memiliki app_metadata.role = \"admin\" di Supabase Auth."
      );
    }
    if (res.status === 404) {
      throw new Error(
        json.message ??
          "Edge function belum di-deploy. Jalankan: supabase login && SKIP_SECRETS=1 bash scripts/supabase-release.sh"
      );
    }
    throw new Error(json.message ?? json.detail ?? json.error ?? "Request failed");
  }
  return json as T;
}
...
export function syncStagingFromStorage(platform: string, version = DEFAULT_VERSION) {
